...
# 从各种地方import 模型进来 需要 1 设置自己的模型 所有的yolo的类别数发生改变
# 使用biggan来生成 好一点的patch 其中的id是imagenet中的类别
# pytorchYOLOv3 Yolov4中提供模型
# adversarialyolo里面提供的是数据集加载和训练 
# style中提供跑的生成器 用于生成对应的图像 但是计算特征中的距离是否有必要呢
...
# patch是否是方形 选择否 需要根据我们的需求重新写

# patch增强设置 是否旋转 随机位置 模糊 
...
# 训练相关设置 patch_sacle 大小 
# 从哪里进行训练 指定攻击类别和生成类别 latent space  
# training setting
# 是否使用预训练的gan网络
...
# 用于保存实验的文件夹，方便训练 这里是参考的yolo进行的设置。。。
# confirm folder
...
# InriaDataset
    ...
    train_loader_second = torch.utils.data.DataLoader(InriaDataset(img_dir='./dataset/inria/Train/pos', 
                                                            lab_dir='./dataset/inria/Train/pos/yolo-labels', 
                                                            max_lab=14,
...
### ---------------------------------------------------------- Checkpoint & Init -------------------------------------------------------------------- ###
# Training preprocess batch长度
epoch_length_second  = len(train_loader_second)
ep_loss_det   = 0
ep_loss_tv    = 0
torch.cuda.empty_cache()
# Create optimizers
# 优化patch 或者latent_shift 如果不使用GAN生成就是直接使用opt_ap 那么对于style——gan 怎么设置的呢
...
# load checkpoint 如果是重新训练gan? 网络就需要先加载
if(retrain_gan):
